...
const dog = () => {
    return fetch('dogs')
        .then(resolve => {
            const dogs = resolve.data.dogs;
            const highestNaughty = Math.max(...dogs.map(dog => dog.naughty));
            return dogs.find(dog => dog.naughty === highestNaughty);
        });
};
